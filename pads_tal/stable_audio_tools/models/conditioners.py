...
from .diffusions.adp import NumberEmbedder
...
from .common.transformer import AbsolutePositionalEmbedding
...
class Conditioner(nn.Module):
    ...
    def forward(self, x: tp.Any, current_step :int=-1, max_step :int=-1) -> tp.Any:
...
class IntConditioner(Conditioner):
    ...
    def forward(self, ints: tp.List[int], device=None, current_step :int=-1, max_step :int=-1) -> tp.Any:
...
class NumberConditioner(Conditioner):
    ...
    def forward(self, floats: tp.List[float], device=None, current_step :int=-1, max_step :int=-1) -> tp.Any:
...
class ListConditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device=None, current_step :int=-1, max_step :int=-1) -> tp.Any:
...
class CLAPTextConditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device: tp.Any = "cuda", current_step :int=-1, max_step :int=-1) -> tp.Any:
...
class CLAPAudioConditioner(Conditioner):
    ...
    def forward(self, audios: tp.Union[torch.Tensor, tp.List[torch.Tensor], tp.Tuple[torch.Tensor]] , device: tp.Any = "cuda", current_step :int=-1, max_step :int=-1) -> tp.Any:
...
class T5Conditioner(Conditioner):
    ...
    def forward(self, texts: tp.List[str], device: tp.Union[torch.device, str], current_step :int=-1, max_step :int=-1) -> tp.Tuple[torch.Tensor, torch.Tensor]:
        ...
        if not self.send_prompt:
            ...
            token_strs = [self.tokenizer.convert_ids_to_tokens(ids) for ids in encoded["input_ids"]]
            return embeddings, attention_mask, token_strs
...
class PretransformConditioner(Conditioner):
    ...
    def forward(self, audio: tp.Union[torch.Tensor, tp.List[torch.Tensor], tp.Tuple[torch.Tensor]], device: tp.Union[torch.device, str], current_step :int=-1, max_step :int=-1) -> tp.Tuple[torch.Tensor, torch.Tensor]:
...
class SourceMixConditioner(Conditioner):
    ...
    def forward(self, sources: tp.List[tp.Dict[str, torch.Tensor]], device: tp.Union[torch.device, str], current_step :int=-1, max_step :int=-1) -> tp.Tuple[torch.Tensor, torch.Tensor]:
...
class MultiConditioner(nn.Module):
    ...
    def forward(self, batch_metadata: tp.List[tp.Dict[str, tp.Any]], device: tp.Union[torch.device, str], current_step :int=-1, max_step :int=-1) -> tp.Dict[str, tp.Any]:
        output = {}
        ...
        for key, conditioner in self.conditioners.items():
            ...
            if key in self.pre_encoded_keys:
                ...
                result = conditioner(conditioner_inputs, device, current_step=current_step, max_step=max_step)
                if getattr(conditioner, 'send_prompt', False): 
                    if isinstance(result, tuple) and len(result) == 3:
                        output[key] = result[:2]
                        output[key + '_tokens'] = result[2] 
                else:
                    output[key] = result
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any], pretransform=None) -> MultiConditioner:
    ...
    send_prompt = config.get('send_prompt', False)
    ...
    for conditioner_info in config["configs"]:
        ...
        if conditioner_type == "t5":
            conditioners[id] = T5Conditioner(send_prompt=send_prompt, **conditioner_config)
