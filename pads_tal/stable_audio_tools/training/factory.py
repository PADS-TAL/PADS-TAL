...
def create_training_wrapper_from_config(model_config, model):
    ...
    if model_type == 'autoencoder':
        from .diffusion_ae import AutoencoderTrainingWrapper
        ...
        return AutoencoderTrainingWrapper(
            ...
            clip_grad_norm_gen=training_config.get("clip_grad_norm_gen", 0.0),
            clip_grad_norm_disc=training_config.get("clip_grad_norm_disc", 0.0),
            clip_grad_print=training_config.get("clip_grad_print", False),
            warmup_mode=training_config.get("warmup_mode", "adv"), 
            ramp_disc_feature_matching=training_config.get('ramp_disc_feature_matching', 0),
            ramp_disc_adv=training_config.get('ramp_disc_adv', 0),
            separate_disc=training_config.get('separate_disc', False),
        )
    elif model_type == 'autoencoder_mvae':
        from .diffusion_mvae import MultiModalVAETrainingWrapper
        ...
        return MultiModalVAETrainingWrapper(
            model, 
            lr=training_config["learning_rate"],
            warmup_steps=training_config.get("warmup_steps", 0), 
            encoder_freeze_on_warmup=training_config.get("encoder_freeze_on_warmup", False),
            ...
            warmup_mode=training_config.get("warmup_mode", "adv"), 
            separate_disc=training_config.get('separate_disc', False),
            decoder_text_freeze=training_config.get('decoder_text_freeze', False),
        )
    elif model_type == 'diffusion_cond':
        
        from .diffusion_base import DiffusionCondTrainingWrapper
        return DiffusionCondTrainingWrapper(
            ...
            mmvae_conditioning = model_config['model']['pretransform']['type']=='autoencoder_mvae',
        )
    else:
        raise NotImplementedError(f'Unknown model type: {model_type}')

def create_demo_callback_from_config(model_config, demo_dir=None, wandb_skip=False, **kwargs):
    model_type = model_config.get('model_type', None)
    assert model_type is not None, 'model_type must be specified in model config'
    ...
    if model_type == 'autoencoder':
        from .diffusion_ae import AutoencoderDemoCallback
        return AutoencoderDemoCallback(
            demo_every=demo_config.get("demo_every", 2000), 
            sample_size=model_config["sample_size"], 
            sample_rate=model_config["sample_rate"],
            demo_dir=demo_dir,
            wandb_skip=wandb_skip,
            **kwargs
        )
    elif model_type == 'autoencoder_mvae':
        from .diffusion_mvae import MultiModalVAEDemoCallback
        return MultiModalVAEDemoCallback(
            demo_every=demo_config.get("demo_every", 2000), 
            sample_size=model_config["sample_size"], 
            sample_rate=model_config["sample_rate"],
            demo_dir=demo_dir,
        ...
        from .diffusion_base import DiffusionCondDemoCallback
