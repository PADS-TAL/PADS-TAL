...
def get_audio_features(sample, audio_data, max_len, data_truncating, data_filling, audio_cfg, require_grad=False, use_seg=False, force_single:int=0):
    """
        ...
        if len(audio_data) > max_len:
            if data_truncating == "rand_trunc":
                ...
                if chunk_frames == total_frames:
                    ...
                    # choose index for each part
                    if not use_seg: 
                        # Calculate ranges for random selection
                        ...
                        mel_overflow = total_frames - chunk_frames
                        margin = mel_overflow * 0.15         # 15% margin of total length
                        idx_front = int(margin)                 # From 15% point
                        idx_middle = mel_overflow // 2         # Middle (50%)
                        idx_back = int(mel_overflow - margin)   # From 85% point
            ...
            use_seg = use_seg and data_truncating != "fusion"
            ...
            if not use_seg:
                if not force_single:
                    ...
                    idx = int(round(idx/48000)) * 48000 
                    audio_data = audio_data[idx: idx + max_len]
                    sample["waveform"] = audio_data
                else:
                    margin = overflow * 0.15         # 15% margin of total length
                    if force_single==1:
                        idx = int(round(margin/ 48000)) * 48000  # From 15% point
                    elif force_single==3:
                        idx = int(round((overflow-margin)/ 48000)) * 48000 # From 85% point
                    else:
                        idx = int(round((overflow//2)/ 48000)) * 48000   # Middle (50%) - keep
                    audio_data = audio_data[idx: idx + max_len]
                    sample["waveform"] = audio_data
            else:
                if not force_single:
                    # Calculate indices for front, middle, back segments
                    margin = overflow * 0.15         # 15% margin of total length
                    
                    idx_front = int(round(margin/ 48000)) * 48000  # From 15% point
                    idx_middle = int(round((overflow//2)/ 48000)) * 48000   # Middle (50%) - keep
                    idx_back = int(round((overflow-margin)/ 48000)) * 48000 # From 85% point

                    # Create three segments
                    audio_front = audio_data[idx_front: idx_front + max_len]
                    audio_middle = audio_data[idx_middle: idx_middle + max_len]
                    audio_back = audio_data[idx_back: idx_back + max_len]

                    # Stack segments for batch processing (shape: [3, max_len])
                    audio_segments = torch.stack([audio_front, audio_middle, audio_back], dim=0)
                    sample["audio_segments"] = audio_segments
                else:
                    margin = overflow * 0.15         # 15% margin of total length
                    if force_single==1:
                        idx = int(round(margin/ 48000)) * 48000  # From 15% point
            ...
            if not use_seg or force_single:
                sample["waveform"] = audio_data
            else: 
                # For short audio, create 3 identical segments
                audio_segments = torch.stack([audio_data, audio_data, audio_data], dim=0)
                sample["audio_segments"] = audio_segments
